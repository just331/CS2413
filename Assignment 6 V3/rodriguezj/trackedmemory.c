...
void newEntry(void * address, my_size_t bytes){
  entry e;
  e.address = address;
  e.bytes = bytes;
  my_size_t pos = t->size;
...
entry * binSearchEntry(entry * entries, my_size_t minloc, 
                       my_size_t maxloc, void * address){
  ...
  my_size_t midloc = (minloc+maxloc)/2;
...
void * trealloc(void *oldaddress, my_size_t bytes){
  initialize();
...
void * tcalloc(my_size_t number, my_size_t tsize){
  initialize();
  my_size_t bytes = number * tsize;
...
void assertNoMemoryLeaks(){
  my_size_t usage = getMemoryUsage();
  if(usage != 0){
    //fprintf(stderr, "\n");
    //fprintf(stderr, "Bytes Allocated: %u\n", usage);
    //fprintf(stderr, "Entries Not Freed: %u\n", t->size);
    int i;
    for(i = 0; i < t->size; i++)
